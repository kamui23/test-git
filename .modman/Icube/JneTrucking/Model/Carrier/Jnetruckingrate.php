...
namespace Icube\JneTrucking\Model\Carrier;

use Magento\Framework\Exception\LocalizedException;
use Magento\Quote\Model\Quote\Address\RateRequest;

class Jnetruckingrate extends \Magento\Shipping\Model\Carrier\AbstractCarrier implements \Magento\Shipping\Model\Carrier\CarrierInterface
{

    protected $_code                 = 'jnetrucking';
    ...
    public function __construct(
        ...
        \Icube\JneTrucking\Model\ResourceModel\Carrier\JnetruckingrateFactory $tablerateFactory,
    ...
    public function collectRates(RateRequest $request)
    {
        ...
        $request->setPackageWeight(ceil($oldWeight));
        ...
        if ($this->_scopeConfig->getValue('carriers/jnetrucking/free_shipping', \Magento\Store\Model\ScopeInterface::SCOPE_STORE) &&
            ($request->getFreeShipping() === true ||
                ($request->getPackageValue() >= $this->_scopeConfig->getValue('carriers/jnetrucking/min_freeshipping_amount', \Magento\Store\Model\ScopeInterface::SCOPE_STORE) &&
                    $request->getPackageWeight() <= $this->_scopeConfig->getValue('carriers/jnetrucking/max_freeshipping_weight', \Magento\Store\Model\ScopeInterface::SCOPE_STORE)))) {

            $method = $this->_rateMethodFactory->create();
            $method->setCarrier($this->_code);
            $method->setCarrierTitle("JNE Trucking");
        ...
        if (!empty($rates)) {
            ...
            foreach ($rates as $rate) {
                if (!empty($rate) && $rate['price'] >= 0) {
                    ...
                    $method->setMethod('jnetruckingrate' . $count++);
